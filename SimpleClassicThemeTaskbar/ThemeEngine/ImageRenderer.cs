...
namespace SimpleClassicThemeTaskbar.ThemeEngine
{
    class ImageRenderer : BaseRenderer
	{
		...
		public override Size GetTaskButtonGroupWindowSize(int buttonCount) => new Size(Config.Default.TaskbarProgramWidth + (taskbuttonGroupWindowBorderSize * 2), ((buttonCount - 1) * (TaskbarHeight - taskbuttonGroupWindowTaskbuttonRealSize.Top + taskbuttonGroupWindowTaskbuttonRealSize.Bottom)) + (taskbuttonGroupWindowBorderSize * 2));
		...
		public override Point GetSystemTrayIconLocation(int index) => new(systemTrayFirstIconPosition.Item1 + (index * (16 + Config.Default.SpaceBetweenTrayIcons)), systemTrayFirstIconPosition.Item2);
		public override int GetSystemTrayWidth(int iconCount) => systemTrayBaseWidth + (iconCount * 16) + (Config.Default.SpaceBetweenTrayIcons * (iconCount - 1));
